...
Cache.prototype.junk = function(itemHash) {
  ...
  Object.keys(this.data[itemHash].taskResults).forEach(function(taskHash) {
    ...
    var permanent = self.data[itemHash].taskResults[taskHash].permanent;
    var cacheFile = self.data[itemHash].taskResults[taskHash].path;
    if (!permanent && fs.existsSync(cacheFile)) {
      fs.unlink(cacheFile);
    }
  });
...
Cache.prototype.clear = function() {
  ...
  // TODO: don't junk permanent files
...
Cache.prototype.complete = function(itemHash, taskHash, cacheFilePath, etag, isPermanent) {
  ...
  if (!isPermanent) isPermanent = undefined;
  this.data[itemHash].taskResults[taskHash] = { path: cacheFilePath, etag: etag, permanent: isPermanent };
...
Cache.prototype.lookup = function(itemHash, taskHash) {
  ...
  //         permanent: (true, or undefined)
  //         etag: (optional HTTP cache tag)
  //       }
  //     }
  //   }
  // }
